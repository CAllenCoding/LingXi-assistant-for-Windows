# 灵犀助手 LingXi-assistant-for-Windows
...
**前提条件：**
1. 启动 `chat_bot.py`
2. 在灵犀云聊网页端添加助手（在添加完后，请立刻截图保存密码）
3. 在 chat_bot.py 的网页上，先将云聊端口改为 5001，点击保存配置。然后输入在灵犀云聊上添加的助手的 id 与密码，点击连接助手。最后再把云聊端口改为 5002，点击保存配置。

**操作步骤：**
1. 确保 `chat_bot.py` 正在运行（默认端口 5004）
2. 在灵犀云聊网页端向助手发送消息
3. 如需查看助手屏幕，在灵犀云聊上点击“实时画面”
4. 如需停止，在灵犀云聊点击实时画面窗口的“❌”
...
**Prerequisites:**
1. Run `chat_bot.py`
2. Add an assistant on the LingXi Cloud Chat web interface (after adding, take a screenshot to save the password immediately)
3. On the chat_bot.py web page, first change the cloud chat port to 5001, click Save Configuration. Then enter the assistant ID and password you added on LingXi Cloud Chat, click Connect Assistant. Finally, change the cloud chat port to 5002 and click Save Configuration.

**Steps:**
1. Ensure `chat_bot.py` is running (default port 5004)
2. Send a message to the assistant on the LingXi Cloud Chat web interface
3. To view the assistant's screen, click "实时画面" (Live Screen) on LingXi Cloud Chat
4. To stop, click the "❌" on the live screen window in LingXi Cloud Chat
